...
static void chassis_feedback_update(chassis_move_t *chassis_move_update)
{
	...
	chassis_move_update->RC_Z_ChassisSpeedRef = chassis_move_update->chassis_rc_ctrl->rc.ch[CHASSIS_WZ_CHANNEL]*CHASSIS_VZ_RC_SEN;//控制旋转
...
//	fp32 sin_yaw = 0.0f, cos_yaw = 0.0f;
	...
	else if(chassis_behaviour_update->chassis_mode==CHASSIS_SEPARATE_GIMBAL){//底盘分离
...
//  chassis_move.rotate_x_offset = 0;
  ...
  float   max = 0;
  //全向算法
  wheel_rpm[0] = (+vx - vy - vz * rotate_ratio_fr) * wheel_rpm_ratio;//轮子转速
